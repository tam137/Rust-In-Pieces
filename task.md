...
##### 1.2.2.2 Stage-0 short-circuit — 🔬 built, verified, measured **negative**. Work in progress.

**Status 2026-08-25: implemented and node-identical, but slower than v0.32.0 in both
configurations. Not shippable as it stands. The working tree is uncommitted.**
...
- `[ ]` If it stays negative: revert the Stage-0 code in `src/search_service.rs`,
  `src/move_gen_service.rs`, `src/config.rs`, `src/game_handler.rs` and `src/threads.rs`, and
  keep `src/search_diag.rs`, the three scripts and this section. The `is_pseudo_legal` /
  `build_stage0_move` pair is reusable by a future picker and is worth keeping either way.
...
Uncommitted working tree on `master`. New: `src/search_diag.rs`, `scripts/measure_stage0.py`,
`scripts/verify_stage0_identity.py`, `scripts/measure_stage0_throughput.py`. Modified:
`Cargo.toml` (feature `search-diag`), `src/main.rs`, `src/config.rs`
(`enable_tt_move_first`, `stage0_history_snapshot`, both default `true`), `src/threads.rs`,
`src/game_handler.rs`, `src/move_gen_service.rs`, `src/search_service.rs`.

Stage 0 stands down when `enable_one_reply_extension` is on, because that gate reads `turns.len`.
