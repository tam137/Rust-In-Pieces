...
`ZobristTable::clear`. Search time to a fixed depth drops by 27% over six positions (4.44 s to
3.23 s, depth 14, best of five, 20-core x86-64 host, 2026-09-07). No search rule and no search or
evaluation parameter was touched; the only behavioural difference is that the regenerated key
values make different positions collide in the table.
