...
### 1. Connected Passed Pawns — ✅ shipped
...
*   **Tasks**:
    - `[x]` Define `connected_passed_pawn_mg: i16` and `connected_passed_pawn_eg: i16` in `Config`.
    - `[x]` Implement detection in `eval_service.rs` (checking if two passed pawns are on adjacent files and either on the same rank or supporting each other).
    - `[x]` Apply the bonus dynamically based on game phase.

### 2. True Outposts for Knights and Bishops — ✅ shipped
...
*   **Tasks**:
    - `[x]` Add `knight_outpost_true_mg: i16`, `knight_outpost_true_eg: i16`, `bishop_outpost_true_mg: i16`, and `bishop_outpost_true_eg: i16` to `Config`.
    - `[x]` Implement outpost square verification (checking that the square is on rank 4-6, supported by a friendly pawn, and the enemy has no pawn that can attack the square).
    - `[x]` Add bonuses for pieces occupying these true outposts.

### 3. Asymmetric Castling and King Safety — ✅ shipped
...
*   **Tasks**:
    - `[x]` Add separate configuration parameters for Kingside and Queenside pawn/piece shields: `king_pawn_shield_kingside: i16`, `king_pawn_shield_queenside: i16`.
    - `[x]` Modify `white_king` and `black_king` evaluations to check if the king is on files a-c (Queenside) versus files f-h (Kingside) and apply the appropriate shield patterns.
...
### 5. Scale Down for Opposite-Colored Bishops Endgames — ✅ shipped
...
*   **Tasks**:
    - `[x]` Add `opposite_bishops_draw_scale: i16` (scaled by 100, e.g. 50 representing 50% scale-down) to `Config`.
    - `[x]` Implement opposite-colored bishops endgame detection (only Kings, Pawns, and exactly 1 Bishop per side on different square colors).
    - `[x]` Scale the final positional + material evaluation down towards 0 if this endgame condition is met.

### 6. Tarrasch Rule (Rook Behind Enemy Passed Pawn) — ✅ shipped
...
*   **Tasks**:
    - `[x]` Add `rook_behind_enemy_passed_pawn_mg: i16` and `rook_behind_enemy_passed_pawn_eg: i16` to `Config`.
    - `[x]` In rook evaluation, if the file contains an enemy passed pawn and the rook is placed behind it (e.g., rook rank is lower than white passed pawn rank, or higher than black passed pawn rank), reward the rook.
