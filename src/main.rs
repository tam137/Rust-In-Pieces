            ...
            match io::stdin().read_line(&mut uci_token) {
                Ok(_) => {
                    if uci_token.trim() == "uci" {
                        ...
                        println!("id name RustInPieces V80_pieces_eval_with_phase_trunc_after_20");
    ...
    loop {
        ...
        if received == "go" {
            ...
            if opening_book_move != "" {
                ...
                let best_move = &search::get_best_move_as_min_max_result(&mut board, config.clone().search_depth + depth_modificator, white, &mut stats, &config);
                board.do_turn(&best_move.get_best_turn());
