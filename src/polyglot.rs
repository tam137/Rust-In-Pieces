...
/// The 781 PolyGlot Zobrist constants, in the order fixed by the PolyGlot book format:
/// 64 squares for each of the 12 piece kinds (768 values), the four castling rights, the
/// eight en passant files and one side-to-move value.
///
/// These constants are part of the file format and have to be reproduced verbatim. They were
/// previously derived from a generator, which produced a self-consistent key that matched no
/// book ever written; `polyglot_key_matches_published_test_vectors` pins them against the
/// keys published with the specification.
static POLYGLOT_RANDOM64: [u64; 781] = [
...
/// Rewrites PolyGlot's castling encoding into the notation UCI expects.
///
/// PolyGlot stores castling as the king moving onto its own rook (`e1h1`, `e1a1`, `e8h8`,
/// `e8a8`), while UCI expects the king's real destination square. Without this translation the
/// engine answers a book position with a move no GUI accepts and no board can play.
///
/// A rook moving from e1 to h1 encodes identically, so the rewrite only applies when the piece
/// standing on the origin square really is the king.
fn normalise_castling(board: &Board, mv: &str) -> String {
    const WHITE_KING_PIECE: u8 = 15;
    const BLACK_KING_PIECE: u8 = 25;

    match mv {
        "e1h1" if board.mailbox[4] == WHITE_KING_PIECE => "e1g1".to_string(),
        "e1a1" if board.mailbox[4] == WHITE_KING_PIECE => "e1c1".to_string(),
        "e8h8" if board.mailbox[60] == BLACK_KING_PIECE => "e8g8".to_string(),
        "e8a8" if board.mailbox[60] == BLACK_KING_PIECE => "e8c8".to_string(),
        _ => mv.to_string(),
    }
}
...
fn map_piece_to_polyglot(piece: u8) -> Option<usize> {
    match piece {
        20 => Some(0),  // Black Pawn
        10 => Some(1),  // White Pawn
        22 => Some(2),  // Black Knight
        12 => Some(3),  // White Knight
        23 => Some(4),  // Black Bishop
        13 => Some(5),  // White Bishop
        21 => Some(6),  // Black Rook
        11 => Some(7),  // White Rook
        24 => Some(8),  // Black Queen
        14 => Some(9),  // White Queen
        25 => Some(10), // Black King
        15 => Some(11), // White King
        _ => None,
    }
}
...
pub fn polyglot_key(board: &Board) -> u64 {
    ...
    // 1. Pieces on board.
    //
    // PolyGlot indexes a square as `8 * row + file` with row 0 being rank 1, which is exactly
    // how `fen_service` lays out the mailbox. The index needs no transformation.
    for sq in 0..64 {
        let piece = board.mailbox[sq];
        if piece != 0 {
            if let Some(pg_piece) = map_piece_to_polyglot(piece) {
                key ^= POLYGLOT_RANDOM64[64 * pg_piece + sq];
            }
        }
    }
    ...
    // 4. Side to move. The specification XORs this constant when White is to move.
    if board.white_to_move {
...
impl PolyglotBook {
    ...
    /// Parses a PolyGlot book from raw bytes. A trailing fragment shorter than one 16-byte
    /// entry is ignored rather than rejected, which is what every other reader does.
    ...
    /// Number of entries held by this book.
    ...
    pub fn get_random_book_move(&self, board: &Board) -> String {
        ...
        if let Ok(choice) = matching.choose_weighted(&mut rng, |e| e.weight as u32) {
            normalise_castling(board, &choice.to_algebraic())
        } else if let Some(first) = matching.first() {
            normalise_castling(board, &first.to_algebraic())
        } else {
            String::new()
        }
    }
...
/// `Performance.bin`, compiled into the binary.
///
/// Embedding the book rather than reading it from disk removes the single largest source of
/// silent asymmetry in matchplay: a build that cannot find its book file plays a different game
/// from one that can, and the difference looks exactly like a strength difference. It also means
/// a freshly built engine has a real opening repertoire with no configuration at all.
static EMBEDDED_BOOK_BYTES: &[u8] = include_bytes!("../Performance.bin");

static EMBEDDED_BOOK: Lazy<PolyglotBook> = Lazy::new(|| PolyglotBook::from_bytes(EMBEDDED_BOOK_BYTES));

/// The book compiled into the engine. Parsed once on first use.
pub fn embedded_book() -> &'static PolyglotBook {
    &EMBEDDED_BOOK
}
...
mod tests {
    ...
    /// The nine positions published with the PolyGlot book format description, together with
    /// their keys. This is the test the module never had: the previous suite built its books
    /// from the engine's own key and was therefore satisfied by any self-consistent hash.
    #[test]
    fn polyglot_key_matches_published_test_vectors() {
        let service = Service::new();
        ...
        for (fen, expected) in vectors {
            ...
            assert_eq!(
                polyglot_key(&board),
                expected,
                "PolyGlot key mismatch for '{}': got {:016x}, expected {:016x}",
                fen,
                polyglot_key(&board),
                expected
            );
    ...
    /// Two of the vectors above differ only in whether the en passant square is reachable by a
    /// pawn of the side to move. A book that XORs the file unconditionally still passes a naive
    /// key test, so the distinction gets its own assertion.
    #[test]
    fn polyglot_key_counts_en_passant_only_when_capturable() {
        let service = Service::new();

        // 1.e4 - the e3 square is set, but no black pawn can take it.
        let unreachable = service.fen.set_fen("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1");
        assert_eq!(polyglot_key(&unreachable), 0x823c9b50fd114196);

        // ...b5b4, c2c4 - the black b4 pawn can take on c3, so the file is part of the key.
        let reachable = service.fen.set_fen("rnbqkbnr/p1pppppp/8/8/PpP4P/8/1P1PPPP1/RNBQKBNR b KQkq c3 0 3");
        let without_ep = service.fen.set_fen("rnbqkbnr/p1pppppp/8/8/PpP4P/8/1P1PPPP1/RNBQKBNR b KQkq - 0 3");
        assert_eq!(polyglot_key(&reachable), 0x3c8123ea7b067637);
        assert_ne!(polyglot_key(&reachable), polyglot_key(&without_ep));
    }

    /// End-to-end check against the book that ships in the binary: the initial position must be
    /// found, and every position reached by following the book must be found again. A wrong key
    /// in any of the four components fails on the very first lookup.
    #[test]
    fn embedded_book_is_reachable_from_the_initial_position() {
        let service = Service::new();
        let book = embedded_book();
        assert!(book.len() > 1000, "embedded book looks empty: {} entries", book.len());

        let mut board = service.fen.set_init_board();
        assert!(
            !book.find_entries(&board).is_empty(),
            "the initial position is not in the embedded book"
        );

        // Follow the highest-weighted move for a fixed number of plies. Every position along the
        // way has to be in the book, which is a far stronger statement than a single key match.
        let mut plies_in_book = 0;
        for _ in 0..12 {
            let entries = book.find_entries(&board);
            let Some(best) = entries.iter().max_by_key(|e| e.weight) else {
                break;
            };
            let mv = normalise_castling(&board, &best.to_algebraic());
            board.do_move(&crate::notation_util::NotationUtil::get_turn_from_notation(&mv));
            plies_in_book += 1;
        }
        assert!(
            plies_in_book >= 8,
            "left the embedded book after only {} plies",
            plies_in_book
        );
    }

    /// `find_entries` binary-searches, which is only correct on a sorted file.
    #[test]
    fn embedded_book_is_sorted_by_key() {
        let book = embedded_book();
        assert!(
            book.entries.windows(2).all(|w| w[0].key <= w[1].key),
            "the embedded book is not sorted by key"
        );
    }

    /// PolyGlot's king-onto-rook castling encoding has to become the UCI destination square,
    /// but only when the piece on the origin square is actually the king.
    #[test]
    fn castling_is_translated_from_polyglot_to_uci_notation() {
        let service = Service::new();

        let white_can_castle = service.fen.set_fen("r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1");
        assert_eq!(normalise_castling(&white_can_castle, "e1h1"), "e1g1");
        assert_eq!(normalise_castling(&white_can_castle, "e1a1"), "e1c1");
        assert_eq!(normalise_castling(&white_can_castle, "e8h8"), "e8g8");
        assert_eq!(normalise_castling(&white_can_castle, "e8a8"), "e8c8");

        // A rook on e1 moving to h1 encodes exactly like white castling short and must survive.
        let rook_on_e1 = service.fen.set_fen("4k3/8/8/8/8/8/8/4R2K w - - 0 1");
        assert_eq!(normalise_castling(&rook_on_e1, "e1h1"), "e1h1");

        // Ordinary moves are returned untouched.
        assert_eq!(normalise_castling(&white_can_castle, "a2a4"), "a2a4");
    }
