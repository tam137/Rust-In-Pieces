...
pub struct Config {
    ...
    /// never reduces. Measured on master over 1000 games per pairing at 1000ms+100ms:
    /// disabling it is worth +23.7 Elo, 95% CI [+8, +40]. Restricting it to deep nodes
    /// (`check_extension_min_depth`) does not recover the loss (-9.7 Elo, CI [-26, +6]).
    /// The measurement was made on the HCE evaluation; the extension's cost is a search
    /// property, but this branch carries its own LMR tuning. See task.md 8.2.
...
mod tests {
    ...
    fn test_config_default_initialization() {
        ...
        assert_eq!(config.aspiration_window_initial_delta, 16);
        assert_eq!(config.aspiration_window_multiplier, 5);
        ...
        assert_eq!(config.lmr_history_bad_threshold, 550);
