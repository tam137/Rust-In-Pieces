...
impl SearchService {
    ...
    pub fn get_moves(
        ...
        // this position instead of starting from an empty table. `task.md` 23.1 on `master`.
        //
...
mod tests {
    ...
    fn test_the_search_tables_survive_a_search_instead_of_being_rebuilt_from_zero() {
        // `task.md` 23.1 on `master`. The killer, history and counter-move tables used to be
        // allocated inside `get_moves`, which the iterative deepening loop in `game_handler.rs`
        // calls once *per depth* -- so the depth-5 search threw away everything depth 4 had
        // learned about this exact position, and only the Transposition Table survived.
        //
        // This test pins the lifetime, not the strength.
        let service = Service::new();
