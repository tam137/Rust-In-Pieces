...
2. **Run Performance Tests (Perft) & Wait for Completion:** ONLY if all unit tests are 100% successful (green) and the code compiles without warnings, execute the performance/ignored tests: `cargo test -- --ignored`. **CRITICAL:** You MUST explicitly wait for this background test execution to finish completely and verify that all ignored tests pass cleanly before proceeding to subsequent steps or running `./build_and_release.sh`. Never trigger release builds while test tasks are running in the background.
